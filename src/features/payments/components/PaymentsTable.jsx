...
const StatusBadge = ({ status }) => {
  // Evitamos errores si status es undefined o null
...
export default function PaymentsTable({ subscriptions }) {
  ...
  // --- FILTRADO DE DATOS (Lógica de Arquitectura) ---
  const filteredData = (subscriptions || []).filter(s => {
    if (activeTab === 'membresias') {
      // Solo mostramos contratos de acceso
      return s.estado === 'activo' || s.estado === 'cancelado';
    }
    // Solo mostramos movimientos de caja (cobros)
    return s.estado === 'recibo_generado' || s.estado === 'recibo';
  });

  return (
    <div className="space-y-8">
      
      {/* 1. SELECTOR DE PESTAÑAS (TABS) - Diseño tipo "GymFlow" */}
      <div className="flex p-1.5 bg-slate-100 w-fit rounded-2xl gap-2 ml-4 mb-2">
        <button
          onClick={() => setActiveTab('membresias')}
          className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] transition-all duration-300 ${
              ...
              <tr>
                <td colSpan="5" className="px-6 py-24 text-center">
                  <p className="text-slate-300 font-black uppercase italic text-sm tracking-[0.2em]">
                    No se han encontrado {activeTab}
                  </p>
                </td>
              </tr>
            )}
